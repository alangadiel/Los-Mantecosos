	...
	if(pcbRemovido != NULL)
	{
		...
		for(z=0;z< list_size(Semaforos);z++){
			...
			if(list_size(Semaforos)> 0){
				...
				list_remove_by_condition(sem->listaDeProcesos->elements,buscarProcesoBloqueadoEnSemaforo); //TODO: destruir elemento (free)
			}
		}
		...
		list_remove_and_destroy_by_condition(PaginasPorProceso, esDelPID, free);
		...
		if(PIDxSocket != NULL && PIDxSocket->socketConsola > 0)
			EnviarMensaje(PIDxSocket->socketConsola,"KILLEADO",KERNEL);
		list_remove_and_destroy_by_condition(PIDsPorSocketConsola, LAMBDA(bool _(void* item){ return ((PIDporSocketConsola*)item)->PID == pcbRemovido->PID; }), free);
...
void accion(void* socket)
{
	...
	list_remove_and_destroy_by_condition(PIDsPorSocketConsola,
